...
const visibleConnections = computed(() =>
  filterConnections(props.connections, searchQuery.value),
);
</script>
...
<template>
  <div class="flex h-full min-h-0 flex-col">
    ...
    <ul v-if="visibleConnections.length > 0" class="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
      <li v-for="connection in visibleConnections" :key="connection.id">
        <button
          type="button"
          class="group flex w-full items-center gap-2 rounded-md px-2 py-2 text-left transition-colors hover:bg-[#F5F1FC]"
        ...
        >
          ...
          />
          <span class="min-w-0 flex-1 truncate text-sm text-[#25262E]">{{ connection.name }}</span>
          <span class="text-[10px] tabular-nums text-[#9A9A9A]">{{ connection.entityKeys.length }}</span>
          <svg
            viewBox="0 0 24 24"
